@php
     ...
     * Parent-brand header.
     *
     * Config is stored in the "Header" Statamic global (Globals -> Header in
     * the control panel). Everything below has a hard-coded fallback so the
     * page always renders even if the global is empty.
     */
    $wlHeader = \Statamic\Facades\GlobalSet::findByHandle('header')?->inDefaultSite();

    $wlCurrent = $wlCurrent ?? ($wlHeader?->get('wl_current') ?? 'whylead');
    $wlHomeUrl = $wlHeader?->get('wl_home_url') ?: 'https://whyleadothers.com';

    // Platform switcher — WhyLead / ACEND / Thriving Boundlessly.
    $wlPlatformRows = collect($wlHeader?->get('platforms') ?? [])
        ->filter(fn ($row) => !empty($row['key']));

    if ($wlPlatformRows->isEmpty()) {
        $wlPlatformRows = collect([
            ['key' => 'whylead', 'label' => 'WhyLead', 'here' => 'Consultancy',
            ...
            ['key' => 'acend', 'label' => 'ACEND', 'here' => 'ACEND',
             ...
             'cta_label' => 'Assess Your Managers', 'cta_url' => 'https://acend.whyleadothers.com'],
            ['key' => 'tb', 'label' => 'Thriving Boundlessly', 'here' => 'Thriving Boundlessly',
             ...
             'cta_label' => 'Request a Demo', 'cta_url' => 'https://www.thriveboundlessly.com'],
        ]);
    }

    $wlPlatforms = $wlPlatformRows->keyBy('key')->map(fn ($row) => [
        'label' => $row['label'] ?? '',
        'here'  => $row['here'] ?? ($row['label'] ?? ''),
        'desc'  => $row['desc'] ?? '',
        'blurb' => $row['blurb'] ?? '',
        'url'   => $row['url'] ?? '#',
        'cta'   => [
    ...
    $wlActive    = $wlPlatforms[$wlCurrent] ?? reset($wlPlatforms);
    $wlHereLabel = $wlActive['here'];
    $wlCta       = $wlActive['cta'];

    // Global navigation groups. Each group is either a dropdown (items) or a
    // direct link (direct_url set, items empty).
    $wlNavRows = collect($wlHeader?->get('nav_groups') ?? [])
    ...
    if ($wlNavRows->isEmpty()) {
        $wlNavRows = collect([
            ...
            ['title' => 'Ideas & Resources', 'direct_url' => '', 'items' => [
                ['label' => 'Leadership Podcast', 'tag' => '', 'url' => 'https://whyleadothers.com/podcast'],
                ['label' => 'Free Leadership Readiness Assessment', 'tag' => 'ACEND', 'url' => 'https://acend.whyleadothers.com/readiness'],
                ['label' => 'Our Insights', 'tag' => '', 'url' => 'https://acend.whyleadothers.com/insights'],
            ]],
            ['title' => 'About', 'direct_url' => url('/about'), 'items' => []],
        ]);
    }

    $wlNav = $wlNavRows->map(function ($row) {
        if (!empty($row['direct_url'])) {
            return ['title' => $row['title'], 'link' => $row['direct_url']];
        }
        return [
            'title' => $row['title'],
    ...
    $wlIsExternal = fn ($url) => is_string($url) && str_starts_with($url, 'http');
@endphp
...
<style>
    #mainNavigationMenu {
        --wl-navy: #0F1B3D;
        --wl-orange: #E8521A;
        --wl-warm: rgb(var(--canvas-color));
        --wl-panel: rgb(var(--card-color));
...
<section id="mainNavigationMenu" class="text-content">
    <div class="wl-inner">
        ...
        <nav class="wl-nav" aria-label="Primary">
            @foreach ($wlNav as $group)
                @if (isset($group['link']))
                    <a href="{{ $group['link'] }}" class="wl-nav-trigger wl-nav-direct"
                        @if ($wlIsExternal($group['link'])) target="_blank" rel="noopener" @endif>
                        {{ $group['title'] }}
                    </a>
                @else
                    <div class="wl-item">
        ...
        @foreach ($wlNav as $group)
            @if (isset($group['link']))
                <div class="wl-m-section">
                    <a href="{{ $group['link'] }}" class="wl-m-link" style="font-size:17px"
                        @if ($wlIsExternal($group['link'])) target="_blank" rel="noopener" @endif>
            ...
            @else
                <div class="wl-m-section">
                    <div class="wl-m-title">{{ $group['title'] }}</div>
                    @foreach ($group['items'] as $item)
                        <a href="{{ $item['url'] }}" class="wl-m-link"
                            @if ($wlIsExternal($item['url'])) target="_blank" rel="noopener" @endif>
                            <span>{{ $item['label'] }}</span>
                            @if (!empty($item['tag']))<span class="wl-tag">{{ $item['tag'] }}</span>@endif
                        </a>
                    @endforeach
                </div>
            @endif
