@php
     ...
     */
    // Statamic: read the current platform from the Header global so editors
    // can switch WhyLead <-> ACEND <-> Thriving Boundlessly in one place.
    $wlCurrent = $wlCurrent
        ?? \Statamic\Facades\GlobalSet::findByHandle('header')?->inDefaultSite()?->get('wl_current')
        ?? 'whylead';
...
<footer id="wlFooter" role="contentinfo" aria-labelledby="wlFooterHeading">
    <div class="wf-inner">
        ...
        {{-- ============================================================
             Latest podcast episode — pulled from the Statamic Podcasts collection.
        ============================================================ --}}
        @php
        ...
        <section class="wf-podcast-wrap wf-reveal" data-delay="4" aria-label="Latest podcast episode">
            ...
            <div class="wf-podcast">
                <a href="{{ url('/podcast/' . $episode->slug) }}"
                    ...
                    aria-label="Listen to episode {{ str_pad($episode->get('number'), 3, '0', STR_PAD_LEFT) }}: {{ $episode->get('title') }}"></a>

                <div class="wf-podcast-art">
                    <img src="{{ $episode->get('image') }}"
                        alt="Episode {{ $episode->get('number') }}: {{ $episode->get('title') }} — cover art"
                        loading="lazy" decoding="async" />
                </div>

                <div class="wf-podcast-body">
                    <div class="wf-podcast-meta">
                        <span>EP {{ str_pad($episode->get('number'), 3, '0', STR_PAD_LEFT) }}</span>
                        @if (! empty($episode->get('featuring')))
                            <span class="wf-dot"></span>
                            <span>FT. {{ strtoupper($episode->get('featuring')) }}</span>
                        @endif
                        @if (! empty($episode->get('date')))
                            <span class="wf-dot"></span>
                            <span>{{ \Carbon\Carbon::parse($episode->get('date'))->format('M Y') }}</span>
                        @endif
                    </div>
                    <div class="wf-podcast-title">{{ $episode->get('title') }}</div>
                    @if (! empty($episode->get('description')))
                        <p class="wf-podcast-excerpt">
                            {{ \Illuminate\Support\Str::limit(strip_tags($episode->get('description')), 180) }}
                        </p>
                    @endif
                </div>
