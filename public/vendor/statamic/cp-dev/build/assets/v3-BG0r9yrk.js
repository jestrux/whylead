...
var Ee = () => {
	...
	setup(e, { emit: n }) {
		...
		watch(() => e.stat.checked, (r) => {
			Q || e.processor.afterOneCheckChanged(e.stat) && n("check", e.stat);
		}), watch(() => e.stat.open, (r) => {
			Q || (n(r ? "open" : "close", e.stat), Ee());
		});
		const i = computed(() => {
			const r = [], s = (b) => {
				var c;
				if (b.parent) {
					let u = (c = b.parent) == null ? void 0 : c.children.indexOf(b);
					do {
						u++;
						let k = b.parent.children[u];
						if (k) {
				...
				return !1;
			}, g = e.rtl ? "right" : "left", m = e.btt ? "top" : "bottom";
			let p = e.stat;
			for (; p;) {
				let b = (p.level - 2) * e.indent + e.treeLineOffset;
				const c = s(p), u = () => {
					r.push({ style: {
						[g]: b + "px",
						[m]: c ? 0 : "50%"
				...
				p === e.stat ? p.level > 1 && u() : c && u(), p = p.parent;
			}
			return r;
		}), o = computed(() => {
			let r = (e.stat.level - 2) * e.indent + e.treeLineOffset;
			return { [e.rtl ? "right" : "left"]: r + "px" };
		}), f = computed(() => {
			var r;
			return ((r = e.stat.parent) == null ? void 0 : r.children) || e.processor.stats;
		}), D = computed(() => f.value.length), h = computed(() => f.value.indexOf(e.stat) + 1);
		return {
			indentStyle: t,
			vLines: i,
			hLineStyle: o,
			ariaAttrs: computed(() => {
				...
				const r = e.stat, s = r.children && r.children.length > 0, g = {
					role: "treeitem",
					"aria-level": r.level,
					"aria-setsize": D.value,
					"aria-posinset": h.value,
					tabindex: r === e.activeDescendant ? 0 : -1
				};
				return s && (g["aria-expanded"] = r.open ? "true" : "false"), r.checked === !0 ? g["aria-checked"] = "true" : r.checked === 0 ? g["aria-checked"] = "mixed" : r.checked === !1 && r.checked, r.draggable === !1 && (g["aria-disabled"] = "true"), g;
			})
		};
	}
}), ce = (e, n) => {
	const t = e.__vccOpts || e;
	for (const [i, o] of n) t[i] = o;
	return t;
}, Pe = { class: "tree-node-inner" };
function _e(e, n, t, i, o, f) {
	return e.table ? (openBlock(), createElementBlock("tr", mergeProps({
		...
		style: e.indentStyle,
		ref: "el"
	}, e.ariaAttrs), [e.treeLine ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [(openBlock(!0), createElementBlock(Fragment, null, renderList(e.vLines, (D) => (openBlock(), createElementBlock("div", {
	...
	}, null, 4)) : createCommentVNode("", !0)], 64)) : createCommentVNode("", !0), createBaseVNode("div", Pe, [renderSlot(e.$slots, "default", { indentStyle: e.indentStyle })])], 16));
}
var Ae = /* @__PURE__ */ ce(Ve, [["render", _e]]);
function Fe(e, n = {}) {
	return makeTreeProcessor(e, {
		...n,
		statHandler(i) {
			return this._statHandler2 && (i = this._statHandler2(i)), ae(n.statHandler, reactive(i));
		},
		statsHandler(i) {
			return ae(n.statsHandler, reactive(i));
		},
		statsFlatHandler(i) {
			return ae(n.statsFlatHandler, reactive(i));
		}
	});
}
function ae(e, n) {
...
var Te = defineComponent({
	...
	computed: {
		...
		visibleStats() {
			...
			let t = e;
			return this.btt && (t = t.slice(), t.reverse()), t.filter((i) => n(i));
		},
		rootChildren() {
			return this.stats;
		}
	...
	methods: {
		...
		addMulti(e, n, t) {
			this.batchUpdate(() => {
				let i = t;
				for (const o of e) this.add(o, n, i), i != null && i++;
			});
		},
		remove: S("remove"),
		removeMulti(e) {
		...
		processor: {
			...
			handler(e) {
				if (e) {
					...
					e._statHandler2 = this.statHandler ? (t) => t.data === this.placeholderData ? t : this.statHandler(t) : null, e.afterSetStat = (t, i, o) => {
						const { childrenKey: f, updateBehavior: D } = this;
						let h = this.valueComputed;
						if (D === "new") {
							if (this.batchUpdateWaiting) return;
							h = this.getData();
						} else if (D === "modify") {
							const d = n(i == null ? void 0 : i.data);
							d.includes(t.data) || d.splice(o, 0, t.data);
						}
						this.batchUpdateWaiting || this._updateValue(h);
					}, e.afterRemoveStat = (t) => {
						var D;
						const { childrenKey: i, updateBehavior: o } = this;
						let f = this.valueComputed;
						if (o === "new") {
							if (this.batchUpdateWaiting) return;
							f = this.getData();
						} else if (o === "modify") arrayRemove(n((D = t.parent) == null ? void 0 : D.data), t.data);
						this.batchUpdateWaiting || this._updateValue(f);
					};
				}
...
}, Ue = ["colspan"], Ke = { class: "drag-placeholder he-tree-drag-placeholder" }, Me = {
	key: 0,
	class: "he-tree-sr-only",
	"aria-live": "polite",
	"aria-atomic": "true"
}, He = ["id"];
function We(e, n, t, i, o, f) {
	const D = resolveComponent("TreeNode"), h = resolveComponent("VirtualList");
	return openBlock(), createBlock(h, {
		...
		"aria-describedby": e.ariaInstructions ? e.ariaInstructionsId : void 0,
		...
		default: withCtx(({ item: d, index: r }) => [createVNode(D, {
			"vt-index": r,
			...
			isPlaceholder: d.data === e.placeholderData,
			onClick: (s) => e.$emit("click:node", d),
			onOpen: n[0] || (n[0] = (s) => e.$emit("open:node", s)),
			onClose: n[1] || (n[1] = (s) => e.$emit("close:node", s)),
			onCheck: n[2] || (n[2] = (s) => e.$emit("check:node", s))
		}, {
			default: withCtx(({ indentStyle: s }) => [d.data === e.placeholderData ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [e.table ? (openBlock(), createElementBlock("td", {
				key: 1,
				style: normalizeStyle(s),
				colspan: e.placeholderColspan
			}, [createBaseVNode("div", Ke, [renderSlot(e.$slots, "placeholder", { tree: e.self })])], 12, Ue)) : (openBlock(), createElementBlock("div", Re, [renderSlot(e.$slots, "placeholder", { tree: e.self })]))], 64)) : renderSlot(e.$slots, "default", {
				key: 1,
				node: d.data,
				stat: d,
				indentStyle: s,
		...
		append: withCtx(() => [
			...
			e.ariaInstructions ? (openBlock(), createElementBlock("div", {
				key: 1,
				id: e.ariaInstructionsId,
				class: "he-tree-sr-only"
			}, toDisplayString(e.ariaInstructions), 9, He)) : createCommentVNode("", !0)
		]),
		_: 3
	}, 8, [
		"class",
...
var ze = /* @__PURE__ */ ce(Te, [["render", We]]);
var l = null, a = null, G, W, T, q, le, C, de, st = {
	get startInfo() {
		return G;
	},
	get targetInfo() {
	...
	methods: {
		...
		_onKeydown(e) {
			...
			const i = this._getVisibleStats(), o = i.indexOf(t);
			if (o === -1) return;
			let f = !1;
			if (e.altKey && !this.disableDrag) {
				const D = t.parent ? t.parent.children : this.stats, h = D.indexOf(t);
				switch (e.key) {
					case "ArrowUp":
						if (h > 0) {
							this.move(t, t.parent || null, h - 1), this.$emit("change");
							const d = t.parent ? t.parent.children : this.stats, r = d.indexOf(t);
							this._announce(`Moved to position ${r + 1} of ${d.length}`), this._focusNode(t);
						}
						f = !0;
						break;
					case "ArrowDown":
						if (h < D.length - 1) {
							this.move(t, t.parent || null, h + 2), this.$emit("change");
							const d = t.parent ? t.parent.children : this.stats, r = d.indexOf(t);
							this._announce(`Moved to position ${r + 1} of ${d.length}`), this._focusNode(t);
						}
						f = !0;
					...
					case "ArrowLeft":
						if (t.parent) {
							const d = t.parent.parent || null, s = (d ? d.children : this.stats).indexOf(t.parent);
							this.move(t, d, s + 1), this.$emit("change");
							const g = d ? d.children : this.stats, m = g.indexOf(t);
							this._announce(`Outdented to level ${t.level}, position ${m + 1} of ${g.length}`), this._focusNode(t);
						}
						f = !0;
						break;
					case "ArrowRight":
						if (h > 0) {
							const d = D[h - 1];
							if (this.isDroppable(d)) {
								d.open = !0;
								const r = d.children.filter((s) => s !== t).length;
								this.move(t, d, r), this.$emit("change"), this._announce(`Indented to level ${t.level}, position ${d.children.indexOf(t) + 1} of ${d.children.length}`), this._focusNode(t);
							}
						}
						f = !0;
						break;
				...
				case "ArrowUp":
					o > 0 && this._focusNode(i[o - 1]), f = !0;
					break;
				case "ArrowDown":
					o < i.length - 1 && this._focusNode(i[o + 1]), f = !0;
					break;
				case "ArrowRight":
					if (t.children && t.children.length > 0 && !t.open) t.open = !0;
					else if (t.children && t.children.length > 0 && t.open) {
						const D = t.children.find((h) => !h.hidden);
						D && this._focusNode(D);
					}
					f = !0;
					break;
				case "ArrowLeft":
					t.children && t.children.length > 0 && t.open ? t.open = !1 : t.parent && this._focusNode(t.parent), f = !0;
					break;
				case "Home":
					i.length > 0 && this._focusNode(i[0]), f = !0;
					break;
				case "End":
					i.length > 0 && this._focusNode(i[i.length - 1]), f = !0;
					break;
				case "Enter":
				case " ":
					this.$emit("click:node", t), f = !0;
					break;
				case "*": {
					const D = t.parent ? t.parent.children : this.stats;
					for (const h of D) h.children && h.children.length > 0 && (h.open = !0);
					f = !0;
					break;
				}
			}
	...
	mounted() {
		const e = (r, s) => {
			let g = !0;
			return l && l !== this && (g = g && this._isMoved), this.table && !this._isDragCopy && (g = g && this._isDragCopy), g && (r.x !== s.x || r.y !== s.y);
		}, n = (r, s) => {
			a.ignoreUpdate(() => {
				if (!a.has(a.placeholderData)) {
					if (a.table) {
						let m = 0;
						const p = a.getRootEl().querySelector("tr");
						if (p) for (const { value: b } of iterateAll(p.children)) css(b, "display") !== "none" && (m += b.colSpan || 1);
						m < 1 && (m = 1), a.placeholderColspan = m;
					}
					a.add(a.placeholderData);
				}
				const g = a.getStat(a.placeholderData);
				a.move(g, r, s);
			});
		}, t = () => {
			const r = this;
			if (r.has(r.placeholderData)) return r.remove(r.getStat(r.placeholderData)), !0;
		}, i = (r) => {
			r ? context.dropEffect = l != null && l.dragCopy ? "copy" : "move" : context.dropEffect = "none";
		}, o = (r) => {
			const s = this;
			r ? i(!0) : s.keepPlaceholder ? s.has(s.placeholderData) || i(!1) : (t(), i(!1));
		};
		let f = {
			x: 0,
			y: 0
		};
		const D = this.getRootEl();
		let h = null;
		const d = () => {
			a != null && a.has(a.placeholderData) && a.ignoreUpdate(() => {
				a.remove(a.getStat(a.placeholderData)), l && (l.dragNode.hidden = !1, l.dragOvering = !1);
			});
		};
		this.ariaInstructions = "Use arrow keys to navigate. Alt plus arrow keys to reorder.", this.treeDraggableInstance = extendedDND(D, {
			beforeDragStart: (r) => {
				if (!context.triggerElement) return;
				let s = this.triggerClass;
				(!s || s.length === 0) && (s = "tree-node");
				let g = toArrayIfNot(s);
				if (h = findParent(findParent(context.triggerElement, (p) => {
					if (hasClassIn(p, g)) return !0;
					if (hasClass(p, "tree-node")) return "break";
				}, {
					withSelf: !0,
					until: D
				}), (p) => {
					if (hasClass(p, "tree-node")) return !0;
				}, {
					withSelf: !0,
					until: D
				}), !!h) {
					if (C = this.getNodeByElement(h), !C) throw "Can't find drag node";
					if (this.isDraggable(C)) return this.$emit("before-drag-start", C), this.$emit("beforeDragStart", C), D;
				}
			},
			onDragStart: (r) => {
				var p, b, c;
				if (!h || !C) return;
				{
					const { x: u, y: k } = h.getBoundingClientRect(), { clientX: w, clientY: E } = r;
					(p = r.dataTransfer) == null || p.setDragImage(h, w - u, E - k);
				}
				le = {
					x: r.clientX,
					y: r.clientY
					...
					if (typeof this.resolveStartMovePoint == "function") return this.resolveStartMovePoint(h);
					{
						let u, k = 0;
						if (this.table) {
							...
							u = {
								x: w.x,
								y: w.y
							}, this.rtl && (u.x = w.right), k = w.height;
						} else if (!this.rtl) u = h.children[0].getBoundingClientRect().toJSON(), k = u.height;
						else {
							const w = h.children[0].getBoundingClientRect();
							u = {
								x: w.right,
								y: w.y
							}, k = w.height;
						}
						return this.btt && (u.y += k), u;
					}
				})(), this.dragOvering = !0;
				const g = l.getSiblings(l.dragNode), m = g.indexOf(C);
				G = {
					tree: l,
					dragNode: C,
					parent: C.parent,
					siblings: g,
					indexBeforeDrop: m
				}, a = this, (b = r.dataTransfer) == null || b.setData("text", `he-tree drag start at ${(/* @__PURE__ */ new Date()).toISOString()}`), l._eachDroppable() || setTimeout(() => {
					C.hidden = !0, n(C.parent, m + 1);
				}, 0), (c = this.ondragstart) == null || c.call(this, r);
			},
			onEnter: (r) => {
				this.$emit("enter", r);
			},
			onLeave: (r) => {
				T = null, this.dragOvering = !1, context.preventDefault = !1, t(), this.$emit("leave", r);
			},
			onDragOver: applyFinally((r) => {
				if (!l) {
					if (!this.onExternalDragOver || this.onExternalDragOver(r) === !1) return;
				...
				let c, u;
				const k = a.getRootEl().querySelectorAll(".tree-node"), w = [];
				k.forEach((v) => {
					!hasClassIn(v, ["drag-placeholder-wrapper", "dragging-node"]) && css(v, "display") !== "none" && w.push(v);
				});
				const E = binarySearch(w, (v) => getBoundingClientRect(v)[p ? "bottom" : "top"] - m.y, { returnNearestIfNoHit: !0 });
				let O = null, P, te;
				E.hit || (E.greater ? p || (O = E.index - 1, w[O] || O++) : p && (O = E.index + 1, w[O] || O--)), O ??= E.index, P = w[O], te = p ? w[O - 1] : w[O + 1], c = P && a.getNodeByElement(P), u = te && a.getNodeByElement(te);
				const { indent: j } = a, L = cacheFunction(() => {
					if (a.table) {
						let v = getBoundingClientRect(P).toJSON();
						const I = j * (c.level - 1);
						return b ? (v.width -= I, v.right -= I) : v.x += I, v;
					} else return getBoundingClientRect(P.firstElementChild);
				}).action, he = cacheFunction(() => p ? m.y > L().y + L().height / 2 : m.y < L().y + L().height / 2).action, ke = cacheFunction(() => p ? !P || O === w.length - 1 && he() : !P || O === 0 && he()).action, ue = cacheFunction(() => b ? m.x - (L().x + L().width) : L().x - m.x).action, Oe = cacheFunction(() => b ? m.x < L().x + L().width - j : m.x > L().x + j).action;
				let V;
				if (ke()) V = 1, c = null;
				else if (c) ue() > 0 ? V = c.level - Math.ceil(ue() / j) : Oe() ? V = c.level + 1 : V = c.level;
				else return;
				u && V < u.level && (V = u.level), (async () => {
					let v, I, _ = !1, U = null;
					const M = async (N) => {
						if (N.open) return !0;
						if (a.dragOpen) if (a.dragOpenDelay) if (U = N, T === N) _ = !0;
						else {
							let B = promisePin();
							T = N;
							const K = N;
							return setTimeout(async () => {
								K !== T ? (_ = !0, B.resolve(!0)) : (a.beforeDragOpen && await a.beforeDragOpen(N), K !== T ? (_ = !0, B.resolve(!0)) : (N.open = !0, U = null, B.resolve(!0)));
							}, a.dragOpenDelay), await B.promise;
						}
						else return a.beforeDragOpen && await a.beforeDragOpen(N), N.open = !0, !0;
						else return !1;
					}, H = async () => {
						if (a.isDroppable(c) && await M(c)) {
							if (_) return;
							v = c, I = null;
						} else return !1;
					}, fe = (N = V) => {
						let B = c, K = [];
						for (; B && B.level >= N;) B = B.parent || null, K.unshift(B);
						let pe = 0;
						for (const ve of K) {
							if (a.isDroppable(ve)) return v = ve, I = K[pe + 1] || c, !0;
							pe++;
						}
						return !1;
					};
					de = c || null, c ? V > c.level ? await H() === !1 && fe(c.level) : fe() === !1 && await H() : a.isDroppable(null) && (v = null), T = U;
					const ge = !!(!_ && (v || v === null));
					return {
						cancelled: _,
						success: ge,
						parent: v,
						index: ge ? I ? (v ? v.children : a.stats).filter((N) => N.data !== a.placeholderData).indexOf(I) + 1 : 0 : -1
					};
				})().then((v) => {
					if (!v.cancelled) {
						if (!v.success) {
						...
						if (a.maxLevel != null && a.maxLevel > 0) {
							let I = 1;
							if (l) {
								const U = l.dragNode;
								let M = 0;
								walkTreeData(U, (H) => {
									H.level > M && (M = H.level);
								}, { childrenKey: CHILDREN }), I = M - U.level + 1;
							}
							if (I + (v.parent ? v.parent.level : 0) > a.maxLevel) {
								o(!1);
								return;
							}
						}
						o(!0), n(v.parent, v.index);
					}
				});
			}, () => {}),
			onDrop: (r) => {
				...
				const s = !l;
				if (!a) return;
				const g = l == null ? void 0 : l.dragNode;
				let m, p = (() => {
					var c;
					let b = !0;
					if (!a.has(a.placeholderData)) b = !1;
					else if (s) m = (c = this.externalDataHandler) == null ? void 0 : c.call(this, r), b = m != null;
					else if (!l.dragCopy) {
						const u = a.getStat(a.placeholderData);
						l === a && u.parent === g.parent && findTreeData(g, (k) => k === u);
					}
					return b;
				})();
				if (p) {
					const b = a.getStat(a.placeholderData), c = a.getSiblings(b);
					W = {
						tree: a,
						dragNode: g,
						parent: b.parent,
						siblings: c,
						indexBeforeDrop: c.indexOf(b)
					};
				}
				(() => {
					if (d(), p) {
						let b = W.indexBeforeDrop;
						l && !l.dragCopy && l === a && G.parent == W.parent && G.indexBeforeDrop < b && b--, l && l !== a && !l._eachDroppable() && l.batchUpdate(() => {
							l.remove(g), l.updateCheck();
						}), a.batchUpdate(() => {
							let c = l == null ? void 0 : l.dragNode, u;
							m ? u = m : l._eachDroppable() && (u = cloneTreeData(l.dragNode.data, { childrenKey: l.childrenKey }), l.dragCopyDataHandler && (u = l.dragCopyDataHandler(u))), u && (a.add(u), c = a.getStat(u)), a.move(c, W.parent, b), a.updateCheck();
						});
					}
					a.$emit("after-drop"), p && (l && (l.dragCopy || l.$emit("change")), a !== l && a.$emit("change"));
				})();
			},
			onDragEnd: (r) => {
				d(), l && (l.dragNode && (l.dragNode.hidden = !1), l.dragNode = null, l.dragOvering = !1, l = null), a = null, T = null, C = null, de = null;
			}
		});
	},
	unmounted() {
